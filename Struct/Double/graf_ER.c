...
Vertex* create_graph_ER(uint N,const double p, uint64_t* num_edges){
     ...
     * double p - probability of existence of an edge between any pair of vertices
     * RETURN
     */
    ...
    uint alloc_k = (uint) ceil( p*N*(2 + 3*sqrt(p*(1-p)*N)) );
	...
	while(iter_vert < N) {
	    // drawing vertices
	    if ( p > random_res53() ) { // if connection exists
...
void bin_logaritmic(uint* array_histogram, uint N, const double basis, char* filename){
  uint iter_histogram = 0;  
  uint cells_in_bin = 1;
  ...
  while (iter_histogram < N){
    ...
    cells_in_bin = ceil( basis * cells_in_bin);
...
void count_graph(uint N, double k_av, uint8_t repeats){
