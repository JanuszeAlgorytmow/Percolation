...
/* generates a random number on [0,1) as a __float128 (but with double precision, just further computations are flt128) */
...
uint binomial_distrib(const uint N, __float128 p){
     ...
     * __float128 p - quad precision probability of existance of an edge
     * RETURN
...
uint poisson_distrib(const uint N, __float128 p){
    /*
     * This function returns random number from Poisson distribution,
     * which can represent a number of edges connected to a single vertex.
     * PARAMS
     * uint16_t N   - number of vertices in a graph
     * __float128 p - quad precision probability of existence of an edge
     * RETURN
     * uint16_t k   - a number of edges connected to a single vertex (from
